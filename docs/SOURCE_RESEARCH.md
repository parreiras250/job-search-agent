...
| [Get on Board](https://www.getonbrd.com/user-manual/get-on-board-s-api) | Official public JSON:API search; no auth | Published LATAM-oriented jobs, company, title, description, location, remote modality, tags, seniority, employment and structured salary | Attribution/direct Get on Board URL retained; first page only; remote is not treated as worldwide eligibility | Low; one GET implemented in 13H.2 | P0 | IMPLEMENTED / VERIFIED |
