    ...
    def fetch(self) -> SourceResult:
        try:
            ...
            return SourceResult(
                ...
                records=[],
                message=f"Greenhouse returned HTTP {error.code}",
            ...
            return SourceResult(
                ...
                records=[],
                message="Greenhouse request timed out",
            )
            ...
            return SourceResult(
                ...
                records=[],
                message=f"Could not connect to Greenhouse: {error.reason}",
            )
        except OSError as error:
            return SourceResult(
                status=SourceStatus.CONNECTION_ERROR,
                records=[],
                message=f"Could not connect to Greenhouse: {error}",
            )
        ...
        if not 200 <= response.status < 300:
            return SourceResult(
                status=SourceStatus.HTTP_ERROR,
                records=[],
                message=f"Greenhouse returned HTTP {response.status}",
        ...
        try:
            ...
            return SourceResult(
                ...
                records=[],
                message="Greenhouse returned invalid JSON",
            )
