...
|---|---|---|---|---|---|---|---|
| [Greenhouse](https://developers.greenhouse.io/job-board.html) | JSON Job Board API; board token | GET no auth; zero-cost public reads | Published jobs, stable IDs, details; direct tenant snapshot is authoritative | High for curated US SaaS companies | Low in the manual pilot; one request per tenant, maximum five tenants | P0 | IMPLEMENTED DIRECT ATS PILOT |
